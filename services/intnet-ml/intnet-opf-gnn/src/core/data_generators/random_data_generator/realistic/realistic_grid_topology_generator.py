...
from core.data_generators.random_data_generator.random_grid_topology_generator import generate_random_bus, generate_random_edge, generate_random_generator, generate_random_load


def generate_realistic_grid_topology(num_buses=10, num_generators=3, num_loads=4, edge_density=0.1) -> GridGraphData:
    ...
    # 2. Generate disconnected trees from generators
    ...
    while len(remaining_buses) > 0:
        ...
        if random.random() < 0.3 and load_count < num_loads:
            new_load = generate_random_load(id=load_count, bus_id=dest_bus_id)
    ...
    for _ in range(int(len(edges) * edge_density)):
        ...
        if bus1.id != bus2.id and (bus1.id, bus2.id) not in graph.edges() and (bus2.id, bus1.id) not in graph.edges():
            new_edge = generate_random_edge(id=len(edges), u=bus1.id, v=bus2.id)
            edges.append(new_edge)
